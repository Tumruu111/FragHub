...
import { validateRegistration, validateUserUpdate } from '../../../lib/validation';
...
export const mutations = {
  createUser: async (_: any, args: { input: CreateUserInput }) => {
    ...
    if (validationError) throw new GraphQLError(validationError);
  ...
  placeOrder: async (
    ...
    if (listing.status === 'out_of_order' || listing.stock < 1)
      throw new GraphQLError('This item is out of stock');
    ...
    const [order] = await prisma.$transaction([
      prisma.order.create({
        data: { userId: caller.id, listingId: args.listingId },
      }),
  ...
  cancelOrder: async (
    ...
    if (order.userId !== caller.id && caller.role !== 'Admin')
      throw new GraphQLError('Forbidden', {
        extensions: { code: 'FORBIDDEN' },
      });
    ...
    if (order.status === 'completed')
      throw new GraphQLError('Completed orders cannot be cancelled');
    const [updated] = await prisma.$transaction([
      ...
      prisma.listing.update({
        ...
        data: { stock: { increment: 1 }, status: 'in_stock' },
      }),
    ]);
    return updated;
  },
