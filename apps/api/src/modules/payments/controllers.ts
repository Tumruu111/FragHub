...
const handleFulfillmentFailure = async (
  ...
  logger.error('Paid but unfulfilled — refund needed', {
  ...
  const listings = await prisma.listing.findMany({
    where: { id: { in: payment.listingIds } },
  });

  await sendEmail({
    to: config.email.adminAlert,
    subject: `⚠ Refund needed — paid but unfulfilled order ${payment.id
      .slice(0, 8)
      .toUpperCase()}`,
...
const fulfillPayment = async (payment: {
  ...
  await prisma.$transaction(async (tx) => {
    for (const listingId of payment.listingIds) {
      ...
      if (updated.count === 0)
        throw new Error(`${listingId} went out of stock`);
...
export const createPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (listings.length !== listingIds.length)
      return res
        .status(400)
        .json({ message: 'One or more listings not found' });

    const outOfStock = listings.find(
      (l) => l.status === 'out_of_order' || l.stock < 1
    );
    if (outOfStock)
      return res
        .status(400)
        .json({ message: `"${outOfStock.title}" is out of stock` });
    ...
    const invoice = await createInvoice({
      paymentId: payment.id,
      amount,
      description: `Veritas Parfums — ${listings
        .map((l) => l.title)
        .join(', ')}`,
    });
...
export const checkPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: err?.message ?? 'Payment check failed' });
  }
};
...
export const cancelPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (payment.qpayInvoiceId) {
      await cancelInvoice(payment.qpayInvoiceId).catch(() => {
        /* best-effort */
      });
    }
